...
ts_3333 <- ts(data$`3333_Equity`, start=c(2017, 1), frequency=365)
...
ts_3333 <- ts(data2$`3333_Equity`, start=c(2017, 1), frequency=365)
...
ts_300118 <- ts(data3$`300118 Equity`, start=c(2017, 1), frequency=365)
...
plot(ts_300118, main="300118 Equity", xlab="Time", ylab="Price", col="green")
logret_002459 <- diff(log(ts_002459))
logret_601012 <- diff(log(ts_601012))
logret_300118 <- diff(log(ts_300118))
...
plot(logret_300118, main="300118 Equity", xlab="Time", ylab="Log Returns", col="green")
summary(logret_002459)
summary(logret_601012)
summary(logret_300118)
#let's do the same for the indepedent variables, sheet "sheet 6"
data4 <- read_excel("/Users/mariano/Documents/01.Università/Exchange semester/Econ 664/dataset econ664.xlsx", sheet = "sheet 6")
